...
> 仅当直连对端是可信反向代理（`TRUSTED_PROXIES`，默认 `127.0.0.1,::1`，支持 CIDR）时才采信 `X-Forwarded-*` 转发头，采信层数由 `PROXY_COUNT`（默认 1）控制；其余来源的转发头一律剥离，防止伪造来源 IP 绕过登录限流。nginx 部署在其他机器时，需将其内网 IP 加入 `TRUSTED_PROXIES`。
...
- [x] 图形验证码（管理员登录）
- [ ] IP 白名单限制
- [x] 登录速率限制（防止暴力破解，默认 15 分钟内 5 次失败锁定，可用 `LOGIN_MAX_ATTEMPTS` / `LOGIN_LOCKOUT_SECONDS` 调整）
- [x] 会话超时自动登出（默认 1 小时滑动过期，`SESSION_TIMEOUT` 配置）
- [x] 登录日志记录（含登录失败记录）
- [x] 管理接口 CSRF 防护
- [x] 登录通知（2FA登录成功后推送通知）
- [ ] 登录失败告警
...
覆盖登录限流、可信代理解析、SECRET_KEY 生成/重建、登出 CSRF、日志净化等安全逻辑。测试使用独立的 `data/test.db`，不影响真实数据库。
